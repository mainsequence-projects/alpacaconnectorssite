...
operator. Its custom image is read from `public/alpaca-navigation-logo.png` at the exact pushed
commit, with `app-window` retained only as the fallback icon key.
